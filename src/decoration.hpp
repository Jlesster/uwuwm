...
// WLR_XDG_TOPLEVEL_DECORATION_V1_MODE_SERVER_SIDE the moment the
// decoration object is created, and re-forces it on every request_mode,
// regardless of what the client asked for -- the same "compositor always
// wins" policy dwl/river-style tiling compositors use, and well within
// what the protocol allows (a compositor is only ever obligated to
// *respond* to set_mode with a configure, never to grant the requested
// mode).
